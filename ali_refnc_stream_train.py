...
from trainers.ali_trainer_refnc import AliReFncStreamTrainer
from dataloaders.ali_odl_dataloader import get_ali_stream_dataloader
from models.ali_pretrain_refnc import PretrainReFnc
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="refnc_train_stream", help='[refnc_train_stream]')
    ...
    parser.add_argument('--device_idx', type=str, default='2', help='Device index')
    ...
    parser.add_argument('--pretrain_refnc_model_pth', type=str, default='/mnt/nfs/luomingxuan/airbench/main_exp3/Ali_refnc_pretrain.pth', help='if need a pretrain model to support')
...
if __name__ == '__main__':
    ...
    pretrain_model = PretrainReFnc(args).to(device)
    pretrain_model.load_state_dict(torch.load(args.pretrain_refnc_model_pth, map_location=device),strict=False)


    stream_trainer = AliReFncStreamTrainer(args, pretrain_model,train_loader,test_loader)
