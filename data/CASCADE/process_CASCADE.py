...
def process_features_with_mapping(df, cate_bin_size):
    ...
    for i in tqdm(range(0, 22), desc="Categorical columns"):
        ...
        for val in unique_values:
            ...
            if str_val == '':
                continue
            mapped_id = stable_hash(str_val, bucket_size)
            mapping_dict[val] = mapped_id
        
        mappings[col] = mapping_dict
        
        df[col] = df[col].map(mapping_dict).fillna(0).astype(np.int64)
...
def preprocess_data_df(data_path,output_path):
...
def batch_preprocess_data(input_dir, output_path):

    txt_files = [f for f in os.listdir(input_dir) if f.endswith(".txt")]
    for filename in tqdm(txt_files):
        data_path = os.path.join(input_dir, filename)
        preprocess_data_df(data_path, output_path)


if __name__ == "__main__":
    input_directory = "/mnt/nfs/luomingxuan/air_data_20250501_20250525_source/air_data/"
    output_file = "./processed_data_expand1.txt"
    batch_preprocess_data(input_directory, output_file)
