...
from trainers.ali_trainer_refnw import AliReFnwStreamTrainer
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="refnw_train_stream", help='[refnw_train_stream]')
    ...
    parser.add_argument('--pretrain_refnw_model_pth', type=str, default='/mnt/nfs/luomingxuan/airbench/main_exp3/Ali_refnc_pretrain.pth', help='if need a pretrain model to support')
...
if __name__ == '__main__':
    ...
    pretrain_model = PretrainReFnw(args).to(device)
    pretrain_model.load_state_dict(torch.load(args.pretrain_refnw_model_pth, map_location=device),strict=False)


    stream_trainer = AliReFnwStreamTrainer(args, pretrain_model,train_loader,test_loader)
