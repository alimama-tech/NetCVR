...
class AliPretrainDdfmTrainer(metaclass=ABCMeta):
...
class AliPretrainTnDpDdfmTrainer(metaclass=ABCMeta):
    def __init__(self, args,model,train_loader,test_loader):
...
class AliDdfmStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,outputs, tn_outputs,dp_outputs,pay_labels,delay_labels,eps=1e-6):
        """
        outputs: shape [B] or [B, 1] - logits from main model
        tn_outputs: shape [B] or [B, 1] - logits from true neagative predictor
        dp_outputs: shape [B] or [B, 1] - logits from delay predictor
        labels: shape [B] - binary labels (0 or 1)
        """
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            loss = self.loss_fn(outputs, tn_outputs,dp_outputs,stream_pay_labels,delay_labels_mask)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
