...
class AliPretrainReFnwTrainer(metaclass=ABCMeta):
...
class AliReFnwStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs, stream_labels,stream_pay_mask,eps=1e-6):
        ...
        cvr_p_no_grad = torch.sigmoid(cvr_logits).clone().detach()
        cvr_pos_loss = (1 + cvr_p_no_grad) * stable_log1pex(cvr_logits)
        cvr_neg_loss = (1 - cvr_p_no_grad) * (1 + cvr_p_no_grad) *(cvr_logits + stable_log1pex(cvr_logits))
    ...
    def cvr_loss_fn(self,cvr_outputs,pretrain_cvr_outputs,stream_labels,stream_pay_mask,eps=1e-6):
        
        cvr_logits = cvr_outputs.view(-1)
        pretrain_cvr_logits = pretrain_cvr_outputs.view(-1)
        stream_labels = stream_labels.float().view(-1)
        stream_pay_mask = stream_pay_mask.view(-1)
        cvr_p_no_grad = torch.sigmoid(pretrain_cvr_logits).clone().detach()
        cvr_pos_loss = (1 + cvr_p_no_grad) * (stable_log1pex(cvr_logits))
        cvr_neg_loss = (1 - cvr_p_no_grad) * (1 + cvr_p_no_grad) * (cvr_logits + stable_log1pex(cvr_logits))
        cvr_loss = torch.mean(cvr_pos_loss * stream_labels * stream_pay_mask + cvr_neg_loss * (1 - stream_labels) * stream_pay_mask)
        clf_loss =  cvr_loss
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            cvr_loss = self.cvr_loss_fn(cvr_outputs,pretrain_cvr_outputs,stream_pay_labels,stream_pay_mask)
            cvr_loss.backward()
            self.optimizer.step()
