...
class AliReDfsnStreamTrainer(metaclass=ABCMeta):
    ...
    def __init__(self, args,pretrain_model,stream_main_model,
                 stream_feature_model,
                 stream_unbias_model,
                 main_loader,
                 feature_loader,
                 unbias_loader,
    ...
    def feature_loss_fn(self,cvr_outputs,net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = torch.sigmoid(cvr_outputs).view(-1).clamp(min=eps, max=1-eps)
        net_cvr_logits = torch.sigmoid(net_cvr_outputs).view(-1).clamp(min=eps, max=1-eps)
        ...
        pos_loss = -torch.log(cvr_logits)
        neg_loss = -torch.log(1-cvr_logits)
        cvr_loss = torch.mean(pos_loss  * stream_pay_labels * stream_pay_mask + neg_loss *  (1 - stream_pay_labels) * stream_pay_mask)

        net_pos_loss = -torch.log(net_cvr_logits)
        net_neg_loss = -torch.log(1-net_cvr_logits)
        net_cvr_loss = torch.mean(net_pos_loss  * stream_net_pay_labels  + net_neg_loss *  (1 - stream_net_pay_labels) )

        loss = cvr_loss +  net_cvr_loss
        return loss

    def cvr_feature_loss_fn(self,cvr_outputs,stream_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = torch.sigmoid(cvr_outputs).view(-1).clamp(min=eps, max=1-eps)
        stream_pay_labels = stream_pay_labels.float().view(-1)
        stream_pay_mask = stream_pay_mask.view(-1)
        pos_loss = -torch.log(cvr_logits)
    ...
    def net_cvr_feature_loss_fn(self,net_cvr_outputs,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
    ...
    def unbias_loss_fn(self,cvr_outputs,net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = torch.sigmoid(cvr_outputs).view(-1).clamp(min=eps, max=1-eps)
        net_cvr_logits = torch.sigmoid(net_cvr_outputs).view(-1).clamp(min=eps, max=1-eps)
        stream_pay_labels = stream_pay_labels.float().view(-1)
        stream_net_pay_labels = stream_net_pay_labels.float().view(-1)
        ...
        p_no_grad = cvr_logits.clone().detach()
        pos_loss = (1 + p_no_grad) * torch.log(cvr_logits)
        neg_loss = (1 - p_no_grad) * (1 + p_no_grad) * (torch.log(1 - cvr_logits))
        cvr_loss = torch.mean(-pos_loss * stream_pay_labels *stream_pay_mask - neg_loss * (1 - stream_pay_labels) * stream_pay_mask)
    ...
    def cvr_unbias_loss_fn(self,cvr_outputs,stream_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = torch.sigmoid(cvr_outputs).view(-1).clamp(min=eps, max=1-eps)
        stream_pay_labels = stream_pay_labels.float().view(-1)
        stream_pay_mask = stream_pay_mask.view(-1)
    ...
    def net_cvr_unbias_loss_fn(self,net_cvr_outputs,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
    ...
    def main_loss_fn(self,cvr_outputs,net_cvr_outputs,unbias_cvr_outputs,unbias_net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = torch.sigmoid(cvr_outputs+unbias_cvr_outputs).view(-1).clamp(min=eps,max=1-eps)
        net_cvr_logits = torch.sigmoid(net_cvr_outputs+unbias_net_cvr_outputs).view(-1).clamp(min=eps,max=1-eps)
        stream_pay_labels = stream_pay_labels.view(-1)
        stream_net_pay_labels = stream_net_pay_labels.view(-1)
        stream_pay_mask = stream_pay_mask.view(-1)
    ...
    def cvr_main_loss_fn(self,cvr_outputs,unbias_cvr_outputs,stream_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = torch.sigmoid(cvr_outputs+unbias_cvr_outputs).view(-1).clamp(min=eps,max=1-eps)
        stream_pay_labels = stream_pay_labels.view(-1)
        stream_pay_mask = stream_pay_mask.view(-1)
        pos_loss = -torch.log(cvr_logits)
    ...
    def net_cvr_main_loss_fn(self,net_cvr_outputs,unbias_net_cvr_outputs,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        net_cvr_logits = torch.sigmoid(net_cvr_outputs).view(-1).clamp(min=eps,max=1-eps)
        stream_net_pay_labels = stream_net_pay_labels.view(-1)
        stream_pay_mask = stream_pay_mask.view(-1)
        net_pos_loss = -torch.log(net_cvr_logits)
    ...
    def refund_loss_fn(self,net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_feature_day_dataloader):
            ...
            cvr_outputs = self.stream_feature_model.cvr_forward(features)
            cvr_loss = self.cvr_feature_loss_fn(cvr_outputs,stream_pay_labels,stream_pay_mask)
            cvr_loss.backward()
        ...
        for i, batch in enumerate(tqdm_unbias_day_dataloader):
            ...
            cvr_outputs = self.stream_unbias_model.cvr_forward(features)
            cvr_loss = self.cvr_unbias_loss_fn(cvr_outputs,stream_pay_labels,stream_pay_mask)
            cvr_loss.backward()
        ...
        for i, batch in enumerate(tqdm_main_day_dataloader):
            ...
            with torch.no_grad():
                ...
                feature_cvr_outputs = self.stream_feature_model.cvr_forward(features)
                feature_net_cvr_outputs = self.stream_feature_model.net_cvr_forward(features)
                unbias_x_embed = self.stream_unbias_model.get_x_embed(features)
                unbias_cvr_outputs = self.stream_unbias_model.cvr_forward(features)
                unbias_net_cvr_outputs = self.stream_unbias_model.net_cvr_forward(features)

            self.main_optimizer.zero_grad()
            cvr_outputs = self.stream_main_model.cvr_forward(features,feature_x_embed.detach(),unbias_x_embed.detach())
            cvr_loss = self.cvr_main_loss_fn(cvr_outputs,unbias_cvr_outputs,stream_pay_labels,stream_pay_mask)
            cvr_loss.backward()
            self.main_optimizer.step()

            self.main_optimizer.zero_grad()
            net_cvr_outputs = self.stream_main_model.net_cvr_forward(features,feature_x_embed.detach(),unbias_x_embed.detach())
            net_cvr_loss = self.refund_loss_fn(net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask)
            net_cvr_loss.backward()
            self.main_optimizer.step()
            loss = cvr_loss + net_cvr_loss
            ...
            with torch.no_grad():
                cvr_fused_outputs = self.stream_main_model.predict(cvr_outputs,feature_cvr_outputs,unbias_cvr_outputs)
                net_fused_outputs = self.stream_main_model.net_cvr_forward(features,feature_x_embed.detach(),unbias_x_embed.detach())
                net_fused_outputs = cvr_fused_outputs*(1-torch.sigmoid(net_fused_outputs))
                cvr_auc = auc_score(pay_labels, cvr_fused_outputs)
                all_metrics["CVR_AUC"] += cvr_auc
                net_cvr_auc = auc_score(net_pay_labels,net_fused_outputs)
                all_metrics["NetCVR_AUC"] += net_cvr_auc
                net_cvr_auc_by_cvr_head = auc_score(net_pay_labels,cvr_fused_outputs)
        ...
        all_metrics["NetCVR_AUC_by_CVR_head"] /= main_total_batches
    ...
    def test(self,day_idx):
        ...
        with torch.no_grad():
            ...
            for batch_idx,batch in enumerate(tqdm_day_dataloader):
                ...
                unbias_cvr_outputs = self.stream_unbias_model.cvr_forward(features)
                unbias_net_cvr_outputs = self.stream_unbias_model.net_cvr_forward(features)
                main_cvr_outputs = self.stream_main_model.cvr_forward(features,feature_x_embed.detach(),unbias_x_embed.detach())
                main_net_cvr_outputs = self.stream_main_model.net_cvr_forward(features,feature_x_embed.detach(),unbias_x_embed.detach())
                
                cvr_fused_outputs = self.stream_main_model.predict(main_cvr_outputs,feature_cvr_outputs,unbias_cvr_outputs)
                net_cvr_fused_outputs = cvr_fused_outputs*(1-torch.sigmoid(main_net_cvr_outputs))
                ...
                stream_model_all_net_pay_preds.extend(net_cvr_fused_outputs.cpu().numpy())
                ...
                with torch.no_grad():
                    ...
                    stream_model_net_cvr_auc = auc_score(net_pay_labels,net_cvr_fused_outputs)
