...
class AliPretrainEsdfmTrainer(metaclass=ABCMeta):
...
class AliPretrainTnDpEsdfmTrainer(metaclass=ABCMeta):
    def __init__(self, args,model,train_loader,test_loader):
...
class AliEsdfmStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,outputs, tn_outputs,dp_outputs,pay_labels,eps=1e-6):
        ...
        pos_weight = (1 + dp_logits).detach()
        neg_weight = ((1 + dp_logits) * tn_logits).detach()
