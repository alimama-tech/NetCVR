...
class AliPretrainVanillaNetPayInwTrainer(metaclass=ABCMeta):
...
class AliVanillaNetPayInwStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        ...
        cvr_loss = torch.mean(cvr_pos_loss * stream_pay_labels * stream_pay_mask + cvr_neg_loss * (1 - stream_pay_labels) * stream_pay_mask)
    ...
    def cvr_loss_fn(self,cvr_outputs,stream_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = cvr_outputs.view(-1)
        stream_pay_labels = stream_pay_labels.float().view(-1)
        cvr_pos_loss = stable_log1pex(cvr_logits)
        cvr_neg_loss = cvr_logits + stable_log1pex(cvr_logits)
