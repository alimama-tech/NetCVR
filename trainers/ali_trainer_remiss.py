...
class AliPretrainReMissTrainer(metaclass=ABCMeta):
...
class AliReMissStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,head1_outputs,head2_outputs,head3_outputs,fused_outputs,net_cvr_outputs,stream_pay_labels,stream_pay_labels2,stream_pay_labels3,stream_net_pay_labels,stream_pay_mask,pos_weight,alpha,eps=1e-6,conversion_rate=0.07):
        ...
        head3_neg_loss = -torch.log(1 - head3_logits) * weight_neg
        fused_pos_loss = -torch.log(fused_logits) * (1 + alpha * pos_weight[0])
        fused_neg_loss = -torch.log(1 - fused_logits) * weight_neg

        head1_loss = torch.mean((head1_pos_loss * stream_pay_labels2 * stream_pay_mask + head1_neg_loss * (1 - stream_pay_labels2)  * stream_pay_mask ))
        head2_loss = torch.mean((head2_pos_loss * stream_pay_labels3  * stream_pay_mask + head2_neg_loss * (1 - stream_pay_labels3) * stream_pay_mask))
        head3_loss = torch.mean((head3_pos_loss * stream_pay_labels  * stream_pay_mask+ head3_neg_loss * (1 - stream_pay_labels) * stream_pay_mask))
        fused_loss = torch.mean((fused_pos_loss * stream_pay_labels  * stream_pay_mask + fused_neg_loss * (1 - stream_pay_labels) * stream_pay_mask))
        ...
        clf_loss = head1_loss + head2_loss + head3_loss + fused_loss + net_cvr_loss
        return clf_loss
    ...
    def cvr_loss_fn(self,head1_outputs,head2_outputs,head3_outputs,fused_outputs,stream_pay_labels,stream_pay_labels2,stream_pay_labels3,stream_pay_mask,pos_weight,alpha,eps=1e-6,conversion_rate=0.07):
        head1_logits = head1_outputs.view(-1).clamp(min=eps, max=1-eps)
        head2_logits = head2_outputs.view(-1).clamp(min=eps, max=1-eps)
        head3_logits = head3_outputs.view(-1).clamp(min=eps, max=1-eps)
        fused_logits = fused_outputs.view(-1).clamp(min=eps, max=1-eps)
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        yesterday_total_samples= 0
        yesterday_cvr_samples = 0
        yesterday_net_cvr_samples = 0
        yesterday_conversion_rate = 0.07
        yesterday_net_conversion_rate = 0.07

        train_stream_day_dataloader = self.train_stream_dataloader.get_day_dataloader(day_idx)
        if day_idx> 0:
            train_stream_yesterday_dataloader = self.train_stream_dataloader.get_day_dataloader(day_idx-1)
            for i, batch in enumerate(train_stream_yesterday_dataloader):
                stream_pay_labels = batch['stream_pay_labels'].to(self.device)
                stream_net_pay_labels = batch['stream_net_pay_labels'].to(self.device)
                yesterday_total_samples+= stream_pay_labels.size(0)
                yesterday_cvr_samples += torch.sum(stream_pay_labels).item()
                yesterday_net_cvr_samples += torch.sum(stream_net_pay_labels).item()
            yesterday_conversion_rate = yesterday_cvr_samples/yesterday_total_samples
            yesterday_net_conversion_rate = yesterday_net_cvr_samples/yesterday_total_samples
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            self.optimizer.zero_grad()
            head1_outputs,head2_outputs,head3_outputs,fused_outputs = self.stream_model.cvr_forward(features)
            cvr_loss = self.cvr_loss_fn(head1_outputs,head2_outputs,head3_outputs,fused_outputs,stream_pay_labels,stream_pay_labels2,stream_pay_labels3,stream_pay_mask,self.miss_global_pos_weight,self.alpha,1e-6)
            cvr_loss.backward()
            self.optimizer.step()
            ...
            with torch.no_grad():
                head1_outputs,head2_outputs,head3_outputs,fused_outputs,net_cvr_outputs = self.stream_model.predict(features)
                cvr_auc = auc_score(pay_labels, fused_outputs)
                all_metrics["CVR_AUC"] += cvr_auc
                net_cvr_auc = auc_score(net_pay_labels,net_cvr_outputs)
                all_metrics["NetCVR_AUC"] += net_cvr_auc
                net_cvr_auc_by_cvr_head = auc_score(net_pay_labels,fused_outputs)
    ...
    def test(self,day_idx):
        ...
        with torch.no_grad():
            ...
            for batch_idx,batch in enumerate(tqdm_day_dataloader):
                ...
                head1_outputs,head2_outputs,head3_outputs,fused_outputs,net_outputs = self.stream_model.predict(features)
                ...
                stream_model_all_net_pay_preds.extend(net_outputs.cpu().numpy())
                ...
                with torch.no_grad():
                    ...
                    stream_model_net_cvr_auc = auc_score(net_pay_labels,net_outputs)
