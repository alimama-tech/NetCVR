...
class AliPretrainFnwTrainer(metaclass=ABCMeta):
...
class AliFnwStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,outputs, pay_labels,eps=1e-6):

        logits = outputs.view(-1)
        z = pay_labels.view(-1)
        p_no_grad = torch.sigmoid(outputs).clone().detach()
        pos_loss = (1 + p_no_grad) * stable_log1pex(logits)
        neg_loss = (1 - p_no_grad) * (1 + p_no_grad) * (logits + stable_log1pex(logits))
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            loss = self.loss_fn(outputs, stream_pay_labels)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
