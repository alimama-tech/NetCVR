...
class AliPretrainBdlBaseTrainer(metaclass=ABCMeta):
...
class AliBdlBaseTrainer(metaclass=ABCMeta):

    def __init__(self, args,pretrained_model,train_loader,test_loader):
        
        self.args=args
        self.bdl_wait_window = args.bdl_wait_window
        self.stream_wait_window = args.stream_wait_window
    ...
    def refund_loss_fn(self,net_cvr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        refund_logits = net_cvr_outputs.view(-1)
        ...
        stream_refund_mask = (1 - stream_pay_mask)>0

        stream_refund_labels = stream_refund_labels.float().view(-1)
        stream_refund_mask = stream_refund_mask.float().view(-1)

        net_cvr_pos_loss = stable_log1pex(refund_logits)
        net_cvr_neg_loss = refund_logits + stable_log1pex(refund_logits)
    ...
    def train(self):
        ...
        train_per_days = len(self.test_loader)/len(self.train_loader)

        train_day = 0
        for day in tqdm(range(len(self.test_loader)), desc="Days"):
            test_day_metrics=self.test(day)
            all_day_metrics.append(test_day_metrics)  
            if (day+1) % train_per_days == 0:
                for epoch_idx in tqdm(range(self.epochs),desc=f"Epoch",leave=True):
                    train_metrics = self.train_one_epoch(epoch_idx,train_day)
                    train_day += 1
