...
class AliPretrainDfsnTrainer(metaclass=ABCMeta):
...
class AliDfsnStreamTrainer(metaclass=ABCMeta):
    ...
    def feature_loss_fn(self,outputs, stream_labels,eps=1e-6):
        ...
        clf_loss = torch.mean(pos_loss  * z + neg_loss *  (1 - z))

        return clf_loss
    
    def unbias_loss_fn(self,outputs, pay_labels,eps=1e-6):
        logits = torch.sigmoid(outputs).view(-1).clamp(min=eps,max=1-eps)
        z = pay_labels.view(-1)
    ...
    def main_loss_fn(self,outputs,unbias_outputs, pay_labels,eps=1e-6):
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_feature_day_dataloader):
            ...
            outputs = self.stream_feature_model(features)
            loss = self.feature_loss_fn(outputs, stream_pay_labels)
            loss.backward()
        ...
        for i, batch in enumerate(tqdm_unbias_day_dataloader):
            ...
            outputs = self.stream_unbias_model(features)
            loss = self.unbias_loss_fn(outputs, stream_pay_labels)
            loss.backward()
        ...
        for i, batch in enumerate(tqdm_main_day_dataloader):
            ...
            outputs = self.stream_main_model(features,feature_x_embed.detach(),unbias_x_embed.detach())
            loss = self.main_loss_fn(outputs,unbias_outputs,stream_pay_labels)
            loss.backward()
            ...
            with torch.no_grad():
                fused_outputs = self.stream_main_model.predict(outputs,feature_outputs,unbias_outputs)
    ...
    def test(self,day_idx):
        ...
        with torch.no_grad():
            ...
            for batch_idx,batch in enumerate(tqdm_day_dataloader):
                ...
                fused_outputs = self.stream_main_model.predict(main_outputs,feature_outputs,unbias_outputs)
