...
class AliPretrainReDeferTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,cvr_labels,net_cvr_outputs,net_cvr_labels,smoothfactor):
        alpha_for_cvr_label_smoothing =  smoothfactor
        smooth_condition = torch.not_equal(cvr_labels, net_cvr_labels)
        cvr_labels_smoothed = (1-alpha_for_cvr_label_smoothing) * cvr_labels
        cvr_labels_smooth_modified = torch.where(smooth_condition, cvr_labels_smoothed, cvr_labels)
        cvr_loss = F.binary_cross_entropy(cvr_outputs, cvr_labels_smooth_modified.float())
    ...
    def train_one_epoch(self,epoch_idx):
        ...
        for batch_idx,batch in enumerate(tqdm_dataloader):
            ...
            cvr_outputs,net_cvr_outputs,smoothfactor,main_feats,aux_feats = self.model(features)
            loss = self.loss_fn(cvr_outputs,pay_labels,net_cvr_outputs,net_pay_labels,smoothfactor)
            loss.backward()
            self.optimizer.step()
...
class AliPretrainDpDrReDeferTrainer(metaclass=ABCMeta):
    def __init__(self, args,model,train_loader,test_loader):
    ...
    def loss_fn(self,dp_logits,dr_logits,pay_labels,delay_pay_labels_afterPay,delay_refund_labels_afterRefund,eps=1e-6):
        pos_labels = pay_labels.float()
        dp_labels = delay_pay_labels_afterPay.float()
        dr_labels = delay_refund_labels_afterRefund.float()

        dp_prob = dp_logits.clamp(min=eps, max=1 - eps)
        dp_loss = F.binary_cross_entropy(
            dp_prob, dp_labels
        )
        dr_prob = dr_logits.clamp(min=eps, max=1 - eps)
        dr_loss = F.binary_cross_entropy(
            dr_prob, dr_labels
        )
        loss =  dp_loss + dr_loss
        return loss
    ...
    def train_one_epoch(self,epoch_idx):
        ...
        for batch_idx,batch in enumerate(tqdm_dataloader):
            ...
            delay_pay_label_afterRefund = batch['delay_pay_label_afterRefund'].to(self.device)
            inw_pay_labels_afterPay = batch['inw_pay_labels_afterPay'].to(self.device)
            delay_refund_labels_afterRefund = batch['delay_refund_label_afterRefund'].to(self.device)
            self.optimizer.zero_grad()
            dp_logits,dr_logits = self.model(features)
            loss = self.loss_fn(dp_logits,dr_logits,pay_labels,delay_pay_labels_afterPay,delay_refund_labels_afterRefund)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
            ...
            with torch.no_grad():
                dp_logits,dr_logits  = self.model.predict(features)
                dp_cvr_auc = auc_score(delay_pay_labels_afterPay,dp_logits)
                all_metrics["DP_CVR_AUC"] += dp_cvr_auc

                dr_rfr_auc = auc_score(delay_refund_labels_afterRefund,dr_logits)
                all_metrics["DR_RFR_AUC"] += dr_rfr_auc

            tqdm_dataloader.set_description('Epoch {}, loss {:.3f} '.format(epoch_idx+1, loss.detach().item()))
        mean_loss = total_loss / total_batches
        self.logger.info(f"Epoch {epoch_idx+1} - Mean Loss: {mean_loss:.5f}")
        all_metrics["DP_CVR_AUC"] /= len(tqdm_dataloader)
        all_metrics["DR_RFR_AUC"] /= len(tqdm_dataloader)
        self.logger.info(f"Epoch {epoch_idx+1} train: DP_CVR_AUC: {all_metrics['DP_CVR_AUC']:.5f}")
        self.logger.info(f"Epoch {epoch_idx+1} train: DR_RFR_AUC: {all_metrics['DR_RFR_AUC']:.5f}")
        self.scheduler.step()
        self.logger.info(f"Epoch {epoch_idx+1} - Learning Rate: {self.optimizer.param_groups[0]['lr']:.8f}")
    ...
    def test(self):
        ...
        self.logger.info(f"DR_RFR_AUC: {all_metrics['DR_RFR_AUC']:.5f}")
...
class AliReDeferStreamTrainer(metaclass=ABCMeta):

    def __init__(self, args,pretrained_model,pretrained_dp_dr_model,train_loader,test_loader):
        ...
        self.pretrained_dp_dr_model=pretrained_dp_dr_model
        self.pretrained_dp_dr_model.to(self.device)
        for param in self.pretrained_model.parameters():
            param.requires_grad = False
        for param in self.pretrained_dp_dr_model.parameters():
            param.requires_grad = False
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs, dp_outputs,dr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        
        cvr_logits = cvr_outputs.view(-1).clamp(min=eps, max=1-eps)
        net_cvr_logits = net_cvr_outputs.view(-1).clamp(min=eps, max=1-eps)
        dp_logits = dp_outputs.view(-1).clamp(min=eps, max=1-eps)
        dr_logits = dr_outputs.view(-1).clamp(min=eps, max=1-eps)
        stream_pay_labels = stream_pay_labels.float().view(-1)
        stream_net_pay_labels = stream_net_pay_labels.float().view(-1)

        cvr_pos_weight = cvr_logits/(cvr_logits - 0.5*dp_logits)
        cvr_pos_weight = torch.clamp(cvr_pos_weight, min=eps)
        cvr_neg_weight = (1 - cvr_logits) / (1 - cvr_logits + 0.5 * dp_logits)
        cvr_pos_weight =cvr_pos_weight.detach()
        cvr_neg_weight = cvr_neg_weight.detach()
        cvr_pos_loss = -torch.log(cvr_logits)
        cvr_neg_loss = -torch.log(1-cvr_logits)
        cvr_loss = torch.mean(cvr_pos_loss * cvr_pos_weight * stream_pay_labels * stream_pay_mask + cvr_neg_loss * cvr_neg_weight * (1 - stream_pay_labels) * stream_pay_mask)
    

        net_cvr_pos_weight = net_cvr_logits / (net_cvr_logits + 0.66 * dr_logits - 0.33 * dp_logits)
        net_cvr_neg_weight = (1 - net_cvr_logits) / (1 - net_cvr_logits + 0.33 * dp_logits - 0.66 * dr_logits)

        net_cvr_pos_weight = net_cvr_logits / (net_cvr_logits + 0.66 * dr_logits - 0.33 * dp_logits)
        net_cvr_neg_weight = (1 - net_cvr_logits) / (1 - net_cvr_logits + 0.33 * dp_logits - 0.66 * dr_logits)
        net_cvr_pos_weight = net_cvr_pos_weight.detach()
        net_cvr_neg_weight = net_cvr_neg_weight.detach()
        net_cvr_pos_loss = -torch.log(net_cvr_logits)
        net_cvr_neg_loss = -torch.log(1-net_cvr_logits)
        net_cvr_loss = torch.mean(net_cvr_pos_loss * net_cvr_pos_weight * stream_net_pay_labels + net_cvr_neg_loss * net_cvr_neg_weight * (1 - stream_net_pay_labels))
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            self.optimizer.zero_grad()
            cvr_outputs,net_cvr_outputs = self.stream_model(features)
            dp_outputs,dr_outputs = self.pretrained_dp_dr_model(features)

   
            loss = self.loss_fn(cvr_outputs,net_cvr_outputs,dp_outputs,dr_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
