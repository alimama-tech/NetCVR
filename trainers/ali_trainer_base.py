...
class AliPretrainBaseTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,cvr_labels,net_cvr_outputs,net_cvr_labels,eps = 1e-6):
    ...
    def cvr_loss_fn(self,cvr_outputs,cvr_labels,eps=1e-6):
    ...
    def net_cvr_loss_fn(self,net_cvr_outputs,net_cvr_labels,eps=1e-6):
        net_cvr_labels = net_cvr_labels.float()
        net_cvr_outputs = net_cvr_outputs.view(-1)
    ...
    def train_one_epoch(self,epoch_idx):
        ...
        for batch_idx,batch in enumerate(tqdm_dataloader):
            ...
            net_cvr_loss = self.net_cvr_loss_fn(net_cvr_outputs,net_pay_labels)
...
class AliBaseStreamTrainer(metaclass=ABCMeta):
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            net_cvr_loss = self.net_cvr_loss_fn(net_cvr_outputs,stream_pay_labels,stream_pay_mask)
            net_cvr_loss.backward()
            self.optimizer.step()
...
class AliBaseStreamTrainer_for_preliminary_experiment_backfill(metaclass=ABCMeta):
    ...
    def net_cvr_loss_fn_only_train_pay_data(self,net_cvr_outputs,stream_labels,stream_pay_mask,eps=1e-6):
        net_cvr_logits = net_cvr_outputs.view(-1)
        stream_labels = stream_labels.float().view(-1)
        net_cvr_pos_loss = stable_log1pex(net_cvr_logits)
        net_cvr_neg_loss = net_cvr_logits + stable_log1pex(net_cvr_logits)

        net_cvr_loss = torch.mean(net_cvr_pos_loss *  stream_labels * stream_pay_mask + net_cvr_neg_loss *(1 - stream_labels) * stream_pay_mask)
        clf_loss = net_cvr_loss 
        return clf_loss
    

    def net_cvr_loss_fn_only_train_refund_data(self,net_cvr_outputs,stream_labels,stream_pay_mask,eps=1e-6):
        net_cvr_logits = net_cvr_outputs.view(-1)
        stream_labels = stream_labels.float().view(-1)
        ...
        net_cvr_loss = torch.mean(net_cvr_pos_loss *  stream_labels * stream_refund_mask + net_cvr_neg_loss *(1 - stream_labels) * stream_refund_mask)
        clf_loss = net_cvr_loss 
        return clf_loss
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            max_true_labels_batch = 6
            target_batch = 12
            if i == 0 :
                curr_stream_features = features.clone()
                curr_net_pay_labels = net_pay_labels.clone()
                curr_dp_labels = delay_pay_labels_afterPay.clone()
                curr_dr_labels = 1 - stream_pay_mask.clone()
                last_update_labels = torch.zeros_like(pay_labels)
            if (i > 0) & (i < max_true_labels_batch):
                curr_stream_features = torch.cat((curr_stream_features,features),dim=0)
                curr_net_pay_labels = torch.cat((curr_net_pay_labels,net_pay_labels),dim=0)
                curr_dp_labels = torch.cat((curr_dp_labels, delay_pay_labels_afterPay),dim=0)
                curr_dr_labels = torch.cat((curr_dr_labels, 1 - stream_pay_mask),dim=0)
            ...
            if i == target_batch :
                ...
                all_net_cvr_hidden_states = self.stream_model.get_net_cvr_hidden_state(curr_stream_features)
                all_net_cvr_hidden_states = all_net_cvr_hidden_states.detach().cpu().numpy()
                all_net_pay_labels = curr_net_pay_labels.cpu().numpy()
                all_dp_labels = curr_dp_labels.cpu().numpy()
                all_dr_labels = curr_dr_labels.cpu().numpy()
                all_last_update_labels = last_update_labels.cpu().numpy()

                hidden_state_list = [vec.tolist() for vec in all_net_cvr_hidden_states]
                df = pd.DataFrame({
                    'all_hidden_states': hidden_state_list,
                    'all_net_pay_labels': all_net_pay_labels,
                    'all_dp_labels': all_dp_labels,
                    'all_dr_labels': all_dr_labels,
                    'all_last_update_labels': all_last_update_labels
                })

                csv_file = '/home/luomingxuan.lmx/AirBench/data/ali/analysis_scripts/样本对比可视化/Preliminary_Experiment_without_train.csv'
                df.to_csv(
                    csv_file,
                    mode='w',
                    header=True,
            ...
            if i < max_true_labels_batch :
                self.optimizer.zero_grad()
                net_cvr_outputs = self.stream_model.net_cvr_forward(features)
                net_cvr_loss = self.net_cvr_loss_fn(net_cvr_outputs,net_pay_labels,stream_pay_mask)
                net_cvr_loss.backward()
                self.optimizer.step()
            elif i == target_batch :
                self.optimizer.zero_grad()
                net_cvr_outputs = self.stream_model.net_cvr_forward(features)
                net_cvr_loss = self.net_cvr_loss_fn_only_train_pay_data(net_cvr_outputs,stream_pay_labels,stream_pay_mask)
                net_cvr_loss.backward()
                self.optimizer.step()
            else:
                self.optimizer.zero_grad()
            ...
            if i == target_batch :
                ...
                csv_file = '/home/luomingxuan.lmx/AirBench/data/ali/analysis_scripts/样本对比可视化/Preliminary_Experiment_with_dp_train.csv'
                df.to_csv(
                    csv_file,
                    mode='w',
                    header=True,
            ...
            if i == target_batch :
                self.optimizer.zero_grad()
                net_cvr_outputs = self.stream_model.net_cvr_forward(features)
                net_cvr_loss = self.net_cvr_loss_fn_only_train_refund_data(net_cvr_outputs,stream_pay_labels,stream_pay_mask)
                net_cvr_loss.backward()
                self.optimizer.step()
            ...
            if i == target_batch :
                ...
                csv_file = '/home/luomingxuan.lmx/AirBench/data/ali/analysis_scripts/样本对比可视化/Preliminary_Experiment_with_dr_train.csv'
                df.to_csv(
                    csv_file,
                    mode='w',
                    header=True,
...
class AliBaseStreamTrainer_for_preliminary_experiment_oracle(metaclass=ABCMeta):
    ...
    def train(self):
        ...
        for day in tqdm(range(len(self.train_loader)), desc="Days"):
            for epoch_idx in tqdm(range(self.epochs),desc=f"Epoch",leave=True):
                train_metrics = self.train_one_epoch_with_groundtruth(epoch_idx,day)
            test_day_metrics=self.test(day)
            all_day_metrics.append(test_day_metrics)  
        avg_metrics = self.aggregate_metrics(all_day_metrics)
    ...
    def train_one_epoch_with_groundtruth(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            if i == target_batch :
                ...
                csv_file = '/home/luomingxuan.lmx/AirBench/data/ali/analysis_scripts/样本对比可视化/Preliminary_Experiment_Oracle_without_train.csv'
                df.to_csv(
                    csv_file,
                    mode='w',
                    header=True,
            ...
            self.optimizer.zero_grad()
            cvr_outputs = self.stream_model.cvr_forward(features)
            cvr_loss = self.cvr_loss_fn(cvr_outputs,pay_labels,stream_pay_mask)
            cvr_loss.backward()
            self.optimizer.step()
            ...
            if i == target_batch :
                ...
                csv_file = '/home/luomingxuan.lmx/AirBench/data/ali/analysis_scripts/样本对比可视化/Preliminary_Experiment_Oracle_with_train.csv'
                df.to_csv(
                    csv_file,
                    mode='w',
                    header=True,
