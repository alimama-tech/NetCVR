...
class AliPretrainInwpayEsdfm2Trainer(metaclass=ABCMeta):
...
class AliEsdfm2StreamTrainer(metaclass=ABCMeta):

    def __init__(self, args,pretrained_model,pretrained_inwpay_tn_model,train_loader,test_loader):
        ...
        self.pretrained_inwpay_tn_model=pretrained_inwpay_tn_model
        self.pretrained_inwpay_tn_model.to(self.device)
        self.pretrained_inwpay_tn_model.eval()
        
        for param in self.pretrained_model.parameters():
            param.requires_grad = False
        for param in self.pretrained_inwpay_tn_model.parameters():
            param.requires_grad = False
    ...
    def loss_fn(self,outputs,pretrain_cvr_outputs,inwpay_outputs,tn_outputs,stream_pay_labels,eps=1e-6):

        cvr_logits = outputs.view(-1)
        inwpay_logtits = inwpay_outputs.view(-1)
        ...
        cvr_loss = torch.mean(pos_loss*pos_weight*stream_pay_labels + neg_loss*neg_weight*(1-stream_pay_labels))
        return cvr_loss
    
    def loss_fn2(self,outputs,pretrain_cvr_outputs,inwpay_outputs,tn_outputs,stream_pay_labels,eps=1e-6):
    
        cvr_logits = outputs.view(-1)
        inwpay_logtits = inwpay_outputs.view(-1)
        pretrain_cvr_logits = pretrain_cvr_outputs.view(-1)
        tn_outputs = tn_outputs.view(-1)
        pretrain_cvr_prob = torch.sigmoid(pretrain_cvr_logits).detach()
        cvr_prob = torch.sigmoid(cvr_logits).detach()
        cvr_prob = pretrain_cvr_prob
        tn_prob = torch.sigmoid(tn_outputs).detach()
        inwpay_prob = torch.sigmoid(inwpay_logtits).detach()
        stream_pay_labels = stream_pay_labels.view(-1)
        pos_weight = 1 + cvr_prob - inwpay_prob
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            pretrain_outputs = self.pretrained_inwpay_tn_model(features)
            inwpay_outputs = pretrain_outputs[:,0].view(-1)
            tn_outputs = pretrain_outputs[:,1].view(-1)
            pretrain_cvr_outputs = self.pretrained_model(features)
            loss = self.loss_fn(outputs,pretrain_cvr_outputs,inwpay_outputs,tn_outputs,stream_pay_labels)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
