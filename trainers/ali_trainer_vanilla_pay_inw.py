...
class AliPretrainVanillaPayInwTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,cvr_labels):
        ...
        cvr_loss = torch.mean(cvr_labels * cvr_pos_loss + (1 - cvr_labels) * cvr_neg_loss)
    ...
    def train_one_epoch(self,epoch_idx):
        ...
        for batch_idx,batch in enumerate(tqdm_dataloader):
            ...
            cvr_outputs = self.model(features)
            loss = self.loss_fn(cvr_outputs,pay_labels)
            loss.backward()
            self.optimizer.step()
            ...
            with torch.no_grad():
                cvr_outputs = self.model.predict(features)
                cvr_auc = auc_score(pay_labels, cvr_outputs)
                all_metrics["CVR_AUC"] += cvr_auc
                net_cvr_auc = auc_score(net_pay_labels,cvr_outputs)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx,batch in enumerate(tqdm_dataloader):
                ...
                delay_pay_label_afterRefund = batch['delay_pay_label_afterRefund'].to(self.device)
                inw_pay_labels_afterPay = batch['inw_pay_labels_afterPay'].to(self.device)
                cvr_outputs = Recmodel.predict(features)
                ...
                all_net_pay_preds.extend(cvr_outputs.cpu().numpy().tolist())
...
class AliVanillaPayInwStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs, stream_labels,eps=1e-6):
        ...
        cvr_loss = torch.mean(cvr_pos_loss * stream_labels  + cvr_neg_loss * (1 - stream_labels) )
        clf_loss =  cvr_loss
        return clf_loss
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            self.optimizer.zero_grad()
            cvr_outputs = self.stream_model(features)

   
            loss = self.loss_fn(cvr_outputs,stream_pay_labels)
            ...
            total_batches += 1
            with torch.no_grad():
                cvr_outputs = self.stream_model.predict(features)
                cvr_auc = auc_score(pay_labels, cvr_outputs)
                all_metrics["CVR_AUC"] += cvr_auc
                net_cvr_auc = auc_score(net_pay_labels,cvr_outputs)
                all_metrics["NetCVR_AUC"] += net_cvr_auc
    ...
    def test(self,day_idx):
        ...
        with torch.no_grad():
            ...
            for batch_idx,batch in enumerate(tqdm_day_dataloader):
                ...
                delay_pay_label_afterRefund = batch['delay_pay_label_afterRefund'].to(self.device)
                inw_pay_labels_afterPay = batch['inw_pay_labels_afterPay'].to(self.device)
                stream_cvr_outputs = stream_model.predict(features)
                pretrained_cvr_outputs= self.pretrained_model.predict(features)

                all_pay_labels.extend(pay_labels.cpu().numpy().tolist())
                all_net_pay_labels.extend(net_pay_labels.cpu().numpy().tolist())
                stream_model_all_pay_preds.extend(stream_cvr_outputs.cpu().numpy().tolist())
                stream_model_all_net_pay_preds.extend(stream_cvr_outputs.cpu().numpy().tolist())
                pretrained_model_all_pay_preds.extend(pretrained_cvr_outputs.cpu().numpy().tolist())
                pretrained_model_all_net_pay_preds.extend(pretrained_cvr_outputs.cpu().numpy().tolist())
                ...
                with torch.no_grad():
                    ...
                    stream_model_net_cvr_auc = auc_score(net_pay_labels,stream_cvr_outputs)
                    ...
                    pretrained_model_net_cvr_auc = auc_score(net_pay_labels,pretrained_cvr_outputs)
