...
class AliPretrainReDeferV1Trainer(metaclass=ABCMeta):
...
class AliReDeferStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs, dp_outputs,dr_outputs,stream_labels,stream_pay_mask,eps=1e-6):
        ...
        cvr_loss = torch.mean(cvr_pos_loss * cvr_pos_weight * stream_labels * stream_pay_mask + cvr_neg_loss * cvr_neg_weight * (1 - stream_labels) * stream_pay_mask)
        ...
        net_cvr_loss = torch.mean(net_cvr_pos_loss * net_cvr_pos_weight * stream_labels + net_cvr_neg_loss * net_cvr_neg_weight * (1 - stream_labels))
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            loss = self.loss_fn(cvr_outputs,net_cvr_outputs,dp_outputs,dr_outputs,stream_pay_labels,stream_pay_mask)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
