...
class AliPretrainReEsdfmTrainer(metaclass=ABCMeta):
...
class AliReEsdfmStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        ...
        cvr_pos_weight = (1 + dp_logits).detach()
        cvr_neg_weight = ((1 + dp_logits) * tn_logits).detach()
        cvr_loss = torch.mean(cvr_pos_loss * stream_pay_labels * stream_pay_mask * cvr_pos_weight + cvr_neg_loss * (1 - stream_pay_labels) * stream_pay_mask * cvr_neg_weight)
