...
class AliPretrainReDdfmTrainer(metaclass=ABCMeta):
...
class AliReDdfmStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,stream_net_pay_labels,delay_labels,stream_pay_mask,eps=1e-6):
        ...
        tn_logits = torch.sigmoid(tn_outputs).view(-1).clamp(min=eps, max=1-eps)
        dp_logits = torch.sigmoid(dp_outputs).view(-1).clamp(min=eps, max=1-eps)

        z = stream_pay_labels.view(-1) 
        d = delay_labels.view(-1)
        p_no_grad = cvr_logits.clone().detach()
        dist_prob = tn_logits.clone().detach()
        dp_prob = dp_logits.clone().detach()
        z1 = dist_prob
        z2 = 1 - p_no_grad + dp_prob
        pos_loss = -torch.log(cvr_logits)
        neg_loss = -torch.log(1 - cvr_logits)
        loss1 = pos_loss
        loss2 = (1-z1) * pos_loss + z1 * neg_loss
        loss3 = pos_loss
        loss4 = (1-z2) * pos_loss + z2 * neg_loss
        cvr_loss = torch.mean((1-d)*(z*(loss1)*stream_pay_mask + (1-z)*(loss2)*stream_pay_mask) + d*(z*loss3*stream_pay_mask + (1-z)*loss4*stream_pay_mask))
    ...
    def cvr_loss_fn(self,cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,delay_labels,stream_pay_mask,eps=1e-6):
    
        cvr_logits = cvr_outputs.view(-1)
        tn_logits = tn_outputs.view(-1)
        dp_logits = dp_outputs.view(-1)
        z = stream_pay_labels.view(-1) 
        d = delay_labels.view(-1)

        p_no_grad = torch.sigmoid(cvr_logits).detach()
        ...
        loss2 = (1-z1) * pos_loss + z1 * neg_loss
        loss3 = pos_loss
        loss4 = torch.abs(1-z2) * pos_loss + z2 * neg_loss
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            pay_labels = batch['pay_labels'].to(self.device)
            net_pay_labels = batch['net_pay_labels'].to(self.device)
            delay_labels_mask = batch['delay_pay_labels_afterPay'].to(self.device)
            ...
            cvr_loss = self.cvr_loss_fn(cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,delay_labels_mask,stream_pay_mask)
            cvr_loss.backward()
            self.optimizer.step()
