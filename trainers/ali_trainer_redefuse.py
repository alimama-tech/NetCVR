...
class AliPretrainReDefuseTrainer(metaclass=ABCMeta):
...
class AliReDefuseStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        ...
        stream_net_labels = stream_net_pay_labels.float().view(-1)

        zi1 = 1 - tn_logits
        zi2 = dp_logits / (dp_logits + 1 - cvr_logits.clone().detach())
        zi = zi1
        one = torch.tensor([1.], device=cvr_logits.device)
        loss1_weight = (one + dp_logits).detach()
        loss2_weight = dp_logits.detach()
        loss3_weight = (one + dp_logits).detach()
        loss4_weight = one.detach()
        loss1 = -torch.log(cvr_logits)
        loss2 = -torch.log(cvr_logits)
        loss3 = -torch.log(1-cvr_logits)
        loss4 = -torch.log(cvr_logits)
        loss1 = loss1 * loss1_weight
        loss2 = zi * loss2 * loss2_weight
        loss3 = (1 - zi) * loss3 * loss3_weight
        loss4 = loss4 * loss4_weight
        cvr_loss = torch.mean(stream_pay_labels * (loss1 + loss4) * stream_pay_mask + (1 - stream_pay_labels) * (loss2 + loss3) * stream_pay_mask)

    
        net_cvr_pos_loss = -torch.log(net_cvr_logits)
        net_cvr_neg_loss = -torch.log(1-net_cvr_logits)
        net_cvr_loss = torch.mean(net_cvr_pos_loss *  stream_net_labels + net_cvr_neg_loss *(1 - stream_net_labels))
        clf_loss = net_cvr_loss + cvr_loss
        return clf_loss

    def cvr_loss_fn(self,cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,stream_pay_mask,eps=1e-6):
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            cvr_loss = self.cvr_loss_fn(cvr_outputs,tn_outputs,dp_outputs,stream_pay_labels,stream_pay_mask)
            cvr_loss.backward()
            self.optimizer.step()
