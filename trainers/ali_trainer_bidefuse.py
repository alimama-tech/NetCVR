...
class AliPretrainBiDefuseTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,inw_logits,dp_logits,delay_pay_labels_afterPay,inw_pay_labels_afterPay,eps=1e-6):
        inw_logits = inw_logits.view(-1).clamp(min=eps, max=1 - eps)
        dp_logits = dp_logits.view(-1).clamp(min=eps, max=1 - eps)
        inw_loss = F.binary_cross_entropy(inw_logits, inw_pay_labels_afterPay)
        outw_loss = F.binary_cross_entropy(dp_logits, delay_pay_labels_afterPay)

        loss = inw_loss + outw_loss
        return loss
    ...
    def train_one_epoch(self,epoch_idx):
        ...
        for batch_idx,batch in enumerate(tqdm_dataloader):
            ...
            loss = self.loss_fn(inw_logits,dp_logits,delay_pay_labels_afterPay,inw_pay_labels_afterPay)
            loss.backward()
            self.optimizer.step()
            ...
            with torch.no_grad():
                cvr_logits = self.model.predict(features)

                all_pay_labels.extend(pay_labels.cpu().numpy().tolist())
                all_net_pay_labels.extend(net_pay_labels.cpu().numpy().tolist())
                all_pay_preds.extend(cvr_logits.cpu().numpy().tolist())
                all_net_pay_preds.extend(cvr_logits.cpu().numpy().tolist())

                cvr_auc = auc_score(pay_labels, cvr_logits)
                all_metrics["CVR_AUC"] += cvr_auc
                net_cvr_auc = auc_score(net_pay_labels,cvr_logits)
                all_metrics["NetCVR_AUC"] += net_cvr_auc
                inw_cvr_auc = auc_score(inw_pay_labels_afterPay,inw_logits)
                all_metrics["Inw_CVR_AUC"] += inw_cvr_auc
        ...
        all_metrics["Inw_CVR_AUC"] /= len(tqdm_dataloader)
        ...
        self.logger.info(f"Epoch {epoch_idx+1} train: Inw_CVR_AUC: {all_metrics['Inw_CVR_AUC']:.5f}")
        self.logger.info(f"Epoch {epoch_idx+1} train: DP_CVR_AUC: {all_metrics['DP_CVR_AUC']:.5f}")
        self.logger.info(f"Epoch {epoch_idx+1} train: Global_CVR_AUC: {all_metrics['Global_CVR_AUC']:.5f}")
        self.logger.info(f"Epoch {epoch_idx+1} train: Global_NetCVR_AUC: {all_metrics['Global_NetCVR_AUC']:.5f}")
        self.scheduler.step()
    ...
    def test(self):
        ...
        self.logger.info(f"Inw_CVR_AUC: {all_metrics['Inw_CVR_AUC']:.5f}")
...
class AliBiDefuseStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,outputs,inw_outputs,dp_outputs,pay_labels,inw_pay_labels_afterPay,delay_pay_labels_afterPay,eps=1e-6):
        inw_logits = inw_outputs.view(-1).clamp(min=eps, max=1-eps)
        outw_logits = dp_outputs.view(-1).clamp(min=eps, max=1-eps)

        cvr_label = pay_labels
        outw_label = delay_pay_labels_afterPay
        inw_mask = inw_pay_labels_afterPay
        inw_pos = -torch.log(inw_logits)
        inw_neg = -torch.log(1 - inw_logits)
        inw_loss = torch.mean((cvr_label * inw_pos + (1 - cvr_label) * inw_neg) ) 

        outw_pos = -torch.log(outw_logits)
        outw_neg = -torch.log(1 - outw_logits)
        outw_loss = torch.mean((outw_label * outw_pos + (1 - outw_label) * outw_neg))
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            inw_outputs = outputs[:,0].view(-1)
            dp_outputs = outputs[:,1].view(-1)
            loss = self.loss_fn(outputs,inw_outputs,dp_outputs,stream_pay_labels,inw_pay_labels_afterPay,delay_pay_labels_afterPay)
            loss.backward()
            self.optimizer.step()
            total_loss += loss.item()
            total_batches += 1
            with torch.no_grad():
                logits = self.stream_model.predict(features)
                cvr_auc = auc_score(pay_labels, logits)
                all_metrics["CVR_AUC"] += cvr_auc
                net_cvr_auc = auc_score(net_pay_labels,logits)
                all_metrics["NetCVR_AUC"] += net_cvr_auc
