...
class AliPretrainDpDrReDeferTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,dp_logits,dr_logits,pay_labels,delay_pay_labels_afterPay,delay_refund_labels_afterRefund,eps=1e-6):
        ...
        dr_pos_loss = stable_log1pex(dr_logits)
        dr_neg_loss = dr_logits + stable_log1pex(dr_logits)
        dr_loss = torch.mean(dr_pos_loss * dr_labels + dr_neg_loss * (1 - dr_labels))
...
class AliReDeferStreamTrainer(metaclass=ABCMeta):
    ...
    def loss_fn(self,cvr_outputs,net_cvr_outputs, dp_outputs,stream_pay_labels,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = cvr_outputs.view(-1)
        net_cvr_logits = net_cvr_outputs.view(-1)
        dp_logits = dp_outputs.view(-1)
        cvr_probs = torch.sigmoid(cvr_logits)
        dp_probs = torch.sigmoid(dp_logits)
        net_cvr_probs = torch.sigmoid(net_cvr_logits)
        stream_pay_labels = stream_pay_labels.float().view(-1)
        stream_net_pay_labels = stream_net_pay_labels.float().view(-1)

        cvr_pos_weight = cvr_probs/(cvr_probs - 0.5*dp_probs)
        cvr_pos_weight = torch.clamp(cvr_pos_weight, min=eps)
        cvr_neg_weight = (1 - cvr_probs) / (1 - cvr_probs + 0.5 * dp_probs)
        ...
        net_pay_mask = torch.not_equal(stream_pay_labels, stream_net_pay_labels)

        net_cvr_loss = torch.mean(net_cvr_pos_loss * stream_net_pay_labels + net_cvr_neg_loss  * (1 - stream_net_pay_labels))
        clf_loss = net_cvr_loss + cvr_loss
        return clf_loss


    def cvr_loss_fn(self,cvr_outputs,dp_outputs,stream_pay_labels,stream_pay_mask,eps=1e-6):
        cvr_logits = cvr_outputs.view(-1)
        dp_logits = dp_outputs.view(-1)
        cvr_probs = torch.sigmoid(cvr_logits)
        dp_probs = torch.sigmoid(dp_logits)
    ...
    def net_cvr_loss_fn(self,net_cvr_outputs,stream_net_pay_labels,stream_pay_mask,eps=1e-6):
        net_cvr_logits = net_cvr_outputs.view(-1)
        stream_net_pay_labels = stream_net_pay_labels.float().view(-1)
    ...
    def train_one_epoch(self,epoch_idx,day_idx):
        ...
        for i, batch in enumerate(tqdm_day_dataloader):
            ...
            cvr_loss = self.cvr_loss_fn(cvr_outputs,dp_outputs,stream_pay_labels,stream_pay_mask)
            cvr_loss.backward()
            self.optimizer.step()
