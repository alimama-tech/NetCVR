...
from trainers.ali_trainer_reoracle import AliReOracleStreamTrainer
from dataloaders.ali_odl_dataloader import get_ali_stream_dataloader
from models.ali_pretrain_reoracle import PretrainReOracle
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="reoracle_train_stream", help='[reoracle_train_stream]')
    ...
    parser.add_argument('--pretrain_reoracle_model_pth', type=str, default='/mnt/nfs/luomingxuan/airbench/main_exp3/Ali_base_pretrain.pth', help='if need a pretrain model to support')
...
if __name__ == '__main__':
    ...
    pretrain_model = PretrainReOracle(args).to(device)
    pretrain_model.load_state_dict(torch.load(args.pretrain_reoracle_model_pth,map_location=device),strict=False)


    stream_trainer = AliReOracleStreamTrainer(args, pretrain_model,train_loader,test_loader)
