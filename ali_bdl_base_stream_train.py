...
from trainers.ali_trainer_bdl_base import AliBdlBaseTrainer
from dataloaders.ali_bdl_dataloader import get_ali_stream_dataloader
from models.ali_pretrain_bdl_base import PretrainBdlBase
...
def parse_args():
    ...
    parser.add_argument('--bdl_wait_window', type=int, default=1, help='bdl cut window size (days)')
    ...
    parser.add_argument('--mode', type=str, default="bdl_train_stream", help='[bdl_train_stream]')
    ...
    parser.add_argument('--pretrain_bdl_base_model_pth', type=str, default='/mnt/nfs/luomingxuan/airbench/main_exp3/Ali_refnc_pretrain.pth', help='if need a pretrain model to support')
...
if __name__ == '__main__':
    ...
    pretrain_model = PretrainBdlBase(args).to(device)
    pretrain_model.load_state_dict(torch.load(args.pretrain_bdl_base_model_pth, map_location=device),strict=False)

    stream_trainer = AliBdlBaseTrainer(args, pretrain_model,train_loader,test_loader)
