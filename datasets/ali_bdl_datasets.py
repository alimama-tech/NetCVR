...
class DataDF(object):
    ...
    def add_bdl_base_duplicate_samples(self):
        ...
        df1[~pay_mask],
        df1[refund_mask],
        df1[pay_not_refund_mask],
        ], axis=0)


        sample_ts = np.concatenate([
            self.click_ts[pay_mask] + self.pay_attr_window,
            self.click_ts[~pay_mask] + self.pay_attr_window,
            self.pay_ts[refund_mask] + self.refund_attr_window,
            self.pay_ts[pay_not_refund_mask] + self.refund_attr_window,
        ])
        click_ts = np.concatenate([
            self.click_ts[pay_mask],
            self.click_ts[~pay_mask],
            self.click_ts[refund_mask],
            self.click_ts[pay_not_refund_mask]
        ])
        ...
        pay_ts = np.concatenate([
            ...
            self.pay_ts[pay_not_refund_mask]
        ])
        ...
        refund_ts = np.concatenate([
            ...
            self.refund_ts[~pay_mask],
            self.refund_ts[refund_mask],
            self.refund_ts[pay_not_refund_mask]
        ])
        ...
        stream_pay_labels = np.concatenate([
            ...
            np.ones((np.sum(pay_not_refund_mask),))], axis=0)
        ...
        stream_pay_mask = np.concatenate([
            ...
            np.ones((np.sum(~pay_mask),)),
            np.zeros((np.sum(refund_mask),)), 
            np.zeros((np.sum(pay_not_refund_mask),))], axis=0)

        pay_labels = np.concatenate([
            pay_labels[pay_mask], 
            pay_labels[~pay_mask], 
            pay_labels[refund_mask],
            pay_labels[pay_not_refund_mask],
            ],axis=0)
        ...
        net_pay_labels = np.concatenate([
            ...
            net_pay_labels[pay_not_refund_mask],
            ],axis=0)
        ...
        refund_labels = np.concatenate([
            ...
            refund_labels[pay_not_refund_mask],
            ],axis=0)
...
def get_ali_bdl_dataset_pretrain(args):
...
def get_ali_bdl_dataset_stream(args):
    """
    ...
    if os.path.isfile(cache_path):
        ...
        test_stream = []
        print("splitting into train and test sets")
        if mode == "bdl_train_stream":
            train_data= data_src.sub_days(0, args.train_split_days_end).add_bdl_base_duplicate_samples()
            train_data= train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)

        for i in np.arange(args.train_split_days_start,args.train_split_days_end,args.bdl_wait_window):
            train_day = train_data.sub_days(i,i+args.bdl_wait_window).shuffle()
