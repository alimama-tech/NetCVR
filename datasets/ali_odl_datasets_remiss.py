...
class DataDF(object):
    ...
    def sub_days(self,start_day,end_day):
        ...
        return DataDF(features = self.features.iloc[mask],
                ...
                stream_pay_labels2= self.stream_pay_labels2[mask],
                stream_pay_labels3 = self.stream_pay_labels3[mask],
    ...
    def add_miss_duplicate_samples(self, pay_wait_window,refund_wait_window):
        """
        ...
        inw_pay_mask2 = np.logical_and(self.pay_ts > 0, (self.pay_ts - self.click_ts) <= pay_wait_window/2)
        inw_pay_mask3 = np.logical_and(self.pay_ts > 0, (self.pay_ts - self.click_ts) <= pay_wait_window/3*2)
    ...
    def add_miss_duplicate_samples1(self, pay_wait_window,refund_wait_window):
        """
        在给定归因窗口和观测窗口内，生成带有延迟标签的样本，并返回DataDF对象。注意此时我们不考虑退款
        miss 有等待窗口，无回补退款样本
    ...
    def add_miss_duplicate_samples2(self, pay_wait_window,refund_wait_window):
        """
        在给定归因窗口和观测窗口内，生成带有延迟标签的样本，并返回DataDF对象。注意此时我们不考虑退款
        miss 有等待窗口，且回补延迟正样本
...
def get_ali_dataset_stream(args):
    ...
    if os.path.isfile(cache_path):
        ...
        if mode =="remiss_train_stream":
            ...
            train_data = data_src.sub_days(0, args.train_split_days_end).add_miss_duplicate_samples(pay_wait_window=pay_wait_window,refund_wait_window=refund_wait_window)
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="remiss_pretrain", help='[remiss_pretrain, remiss_train_stream]')
