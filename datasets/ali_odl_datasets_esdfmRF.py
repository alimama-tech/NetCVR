...
class DataDF(object):
    ...
    def __init__(self, features, click_ts, pay_ts, refund_ts, sample_ts=None, pay_labels=None,net_pay_labels =None,refund_labels=None,stream_pay_labels=None,stream_net_pay_labels=None,stream_refund_labels=None,stream_pay_mask =None,delay_pay_labels_afterPay=None, delay_pay_label_afterRefund = None,delay_refund_label_afterRefund=None,inw_pay_labels_afterPay= None, inw_pay_labels_afterRefund=None, inw_refund_labels_afterRefund=None,pay_attr_window = None, refund_attr_window = None):
        """
        初始化方法。
        Args:
            ...
            stream_refund_labels (numpy.ndarray, optional): 流退款标签数组，默认为 None。如果为 None，则使用退款标签作为流退款标签。
            ...
            inw_refund_labels_afterRefund (numpy.ndarray, optional): 在退款窗口内考虑退款后的退款标签数组，默认为 None。如果为 None，则使用退款标签作为在退款窗口内考虑退款后的退款标签。
        ...
        self.inw_refund_labels_afterRefund = inw_refund_labels_afterRefund

        self.stream_pay_labels = stream_pay_labels
        self.stream_net_pay_labels = stream_net_pay_labels
        self.stream_refund_labels = stream_refund_labels
        ...
        if self.inw_refund_labels_afterRefund is None:
            self.inw_refund_labels_afterRefund = self.refund_labels
        ...
        if self.stream_refund_labels is None:
            self.stream_refund_labels = self.refund_labels
    ...
    def sub_days(self,start_day,end_day):
        ...
        return DataDF(features = self.features.iloc[mask],
                ...
                inw_refund_labels_afterRefund = self.inw_refund_labels_afterRefund[mask],
    ...
    def sub_days_inws_tn_pay(self, start_day, end_day, pay_wait_window):
        """
    ...
    def sub_days_inws_tn_refund(self, start_day, end_day, refund_wait_window):
        start_ts = start_day*SECONDS_A_DAY
        end_ts = end_day*SECONDS_A_DAY
        mask = np.logical_and(self.sample_ts >= start_ts,self.sample_ts < end_ts)
        pos_mask = self.pay_labels>0
        mask = mask & pos_mask
        if self.refund_attr_window is not None:
            diff = self.refund_ts - self.pay_ts
            inw_refund_mask = np.logical_and(self.refund_ts > 0, diff < refund_wait_window)
            delay_refund_mask = np.logical_and(self.refund_ts > 0, np.logical_and(diff > refund_wait_window, diff < self.refund_attr_window))
        else:
            inw_refund_mask = np.logical_and(self.refund_ts > 0, self.refund_ts - self.pay_ts < refund_wait_window)
            delay_refund_mask = np.logical_and(self.refund_ts > 0, self.refund_ts - self.pay_ts > refund_wait_window)
        
        inw_refund_labels_afterRefund = copy.deepcopy(self.refund_labels)
        inw_refund_labels_afterRefund[~inw_refund_mask] = 0

        delay_refund_label_afterRefund = copy.deepcopy(self.refund_labels)
        delay_refund_label_afterRefund[~delay_refund_mask] = 0
        ...
        return DataDF(features = self.features.iloc[mask],
                      ...
                      net_pay_labels = self.net_pay_labels[mask],
                      refund_labels = self.refund_labels[mask],
                      inw_refund_labels_afterRefund = inw_refund_labels_afterRefund[mask],
                      delay_refund_label_afterRefund = delay_refund_label_afterRefund[mask],
                      stream_pay_labels = self.stream_pay_labels[mask],
                      stream_net_pay_labels = self.stream_net_pay_labels[mask],
                      stream_pay_mask= self.stream_pay_mask[mask],
                      pay_attr_window=self.pay_attr_window,
    ...
    def add_esdfmRF_duplicate_samples(self, pay_wait_window,refund_wait_window):
        ...
        sample_ts_delay_pay_mask = np.where(
            inw_refund_ts[delay_pay_mask] > 0,
            self.refund_ts[delay_pay_mask],
            self.pay_ts[delay_pay_mask] + refund_wait_window
        )
    ...
    def add_reesdfm_duplicate_samples(self, pay_wait_window,refund_wait_window):
        ...
        net_pay_labels_inw_pay[refund_mask] = 1
        stream_net_pay_labels = np.concatenate([
        np.ones((np.sum(inw_pay_mask),)),  
        np.zeros((np.sum(~inw_pay_mask),)),
        np.ones((np.sum(delay_pay_mask),)),     
    ...
    def add_esdfmRF_duplicate_samples_v2(self, pay_wait_window,refund_wait_window):
    
        inw_pay_mask = np.logical_and(self.pay_ts > 0, (self.pay_ts - self.click_ts) <= pay_wait_window)
        inw_refund_mask =np.logical_and(self.refund_labels, (self.refund_ts - self.pay_ts) <= refund_wait_window)
        not_inw_refund_mask = np.logical_and(~inw_refund_mask,self.pay_labels>0)
        delay_pay_mask = np.logical_and(np.logical_and(self.pay_ts > 0, (self.pay_ts - self.click_ts) > pay_wait_window),self.pay_labels)
        ...
        df1[delay_pay_mask],       
        df1[inw_refund_mask],
        df1[not_inw_refund_mask],
        df1[delay_refund_mask],       
        ])
        ...
        sample_ts = np.concatenate([
            ...
            self.click_ts[~inw_pay_mask] + pay_wait_window,   
            self.pay_ts[delay_pay_mask],
            self.refund_ts[inw_refund_mask],
            self.pay_ts[not_inw_refund_mask] + refund_wait_window,
            self.refund_ts[delay_refund_mask],    
        ], axis=0)
        ...
        click_ts = np.concatenate([
            ...
            self.click_ts[inw_refund_mask],
            self.click_ts[not_inw_refund_mask],
            self.click_ts[delay_refund_mask],
            ], axis=0)
        pay_ts = np.concatenate([
            self.pay_ts[inw_pay_mask], 
            self.pay_ts[~inw_pay_mask],
            self.pay_ts[delay_pay_mask], 
            self.pay_ts[inw_refund_mask],
            self.pay_ts[not_inw_refund_mask],
            self.pay_ts[delay_refund_mask]], axis=0)
        ...
        refund_ts = np.concatenate([
            ...
            self.refund_ts[delay_pay_mask], 
            self.refund_ts[inw_refund_mask],
            self.refund_ts[not_inw_refund_mask],
            self.refund_ts[delay_refund_mask]], axis=0)
        ...
        np.zeros((np.sum(~inw_pay_mask),)),     
        np.ones((np.sum(delay_pay_mask),)),
        np.ones((np.sum(inw_refund_mask),)),
        np.ones((np.sum(not_inw_refund_mask),)),
        np.ones((np.sum(delay_refund_mask),)),     
        ], axis=0)
        ...
        np.zeros((np.sum(~inw_pay_mask),)),     
        np.ones((np.sum(delay_pay_mask),)),
        np.zeros((np.sum(inw_refund_mask),)),
        np.ones((np.sum(not_inw_refund_mask),)),
        np.zeros((np.sum(delay_refund_mask),)),    
        ], axis=0)
        ...
        np.ones((np.sum(delay_pay_mask),)),
        np.zeros((np.sum(inw_refund_mask),)),
        np.zeros((np.sum(not_inw_refund_mask),)),
        np.zeros((np.sum(delay_refund_mask)))
        ], axis=0)
        ...
        pay_labels[~inw_pay_mask],                
        pay_labels[delay_pay_mask], 
        pay_labels[inw_refund_mask],
        pay_labels[not_inw_refund_mask],
        pay_labels[delay_refund_mask]
        ], axis=0)
        ...
        net_pay_labels[~inw_pay_mask],     
        net_pay_labels[delay_pay_mask], 
        net_pay_labels[inw_refund_mask],
        net_pay_labels[not_inw_refund_mask],
        net_pay_labels[delay_refund_mask],     
        ], axis=0)
        ...
        refund_labels[~inw_pay_mask],          
        refund_labels[delay_pay_mask], 
        refund_labels[inw_refund_mask],
        refund_labels[not_inw_refund_mask],
        refund_labels[delay_refund_mask],          
        ], axis=0)
    ...
    def add_esdfmRF_duplicate_samples_wo_delay_refund_backfill(self, pay_wait_window,refund_wait_window):
        ...
        pay_ts = np.concatenate([
            ...
            self.pay_ts[delay_pay_mask], 
            self.pay_ts[inw_refund_mask],
            self.pay_ts[not_inw_refund_mask],], axis=0)
        ...
        refund_ts = np.concatenate([
            ...
            self.refund_ts[delay_pay_mask], 
            self.refund_ts[inw_refund_mask],
            self.refund_ts[not_inw_refund_mask],], axis=0)
    ...
    def add_esdfmRF_duplicate_samples_wi_refund_bias(self, pay_wait_window,refund_wait_window):
        ...
        pay_mask = np.logical_and(self.pay_ts > 0, self.pay_labels)
        refund_mask = np.logical_and(self.refund_ts > 0, self.refund_labels)
        ...
        click_ts = np.concatenate([
            ...
            self.click_ts[~inw_pay_mask],
            self.click_ts[delay_pay_mask],
            self.click_ts[pay_mask],
            self.click_ts[refund_mask],
            ], axis=0)
        pay_ts = np.concatenate([
            self.pay_ts[inw_pay_mask], 
            self.pay_ts[~inw_pay_mask],
    ...
    def add_esdfmRF_duplicate_samples_dp(self, pay_wait_window,refund_wait_window):
    ...
    def shuffle(self):
        ...
        return DataDF(features = self.features.iloc[idx],
                      ...
                      inw_refund_labels_afterRefund= self.inw_refund_labels_afterRefund[idx],
...
def get_ali_dataset_pretrain(args):
    ...
    if os.path.isfile(cache_path):
        ...
        if mode == "esdfmRF_inws_tn_refund_pretrain":
            train_data = data_src.sub_days_inws_tn_refund(args.train_split_days_start, args.train_split_days_end,refund_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_refund(args.test_split_days_start, args.test_split_days_end,refund_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_inws_tn_pay_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_inws_pay_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_pretrain":
            ...
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_PLE_pretrain":
            train_data = data_src.sub_days(args.train_split_days_start, args.train_split_days_end).shuffle()
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_PLE_v2_pretrain":
            train_data = data_src.sub_days(args.train_split_days_start, args.train_split_days_end).shuffle()
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_PLE_v3_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_PLE_DpTnCl_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_PLE_test_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode == "esdfmRF_PLE_TimeCode_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode == "esdfmRF_PLE_TimeCode_v2_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode == "esdfmRF_PLE_DelayTimeAware_pretrain":
            train_data = data_src.sub_days_inws_tn_pay(args.train_split_days_start, args.train_split_days_end,pay_wait_window).shuffle()
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
            data = {"train": train_data, "test": test_data}
        elif mode =="esdfmRF_ShareEmb_pretrain":
            train_data = data_src.sub_days(args.train_split_days_start, args.train_split_days_end).shuffle()
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
            data = {"train": train_data, "test": test_data}
        else:
    ...
    return {
        "train": {
            ...
            "inw_refund_labels_afterRefund": train_data.inw_refund_labels_afterRefund,
        ...
        "test": {
            ...
            "inw_refund_labels_afterRefund":test_data.inw_refund_labels_afterRefund,
...
def get_ali_dataset_stream(args):
    ...
    if os.path.isfile(cache_path):
        ...
        test_stream = []
        print("splitting into train and test sets")
        if mode =="esdfmRF_train_stream":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        elif mode =="esdfmRF_train_stream_v2":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_v2(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        
        elif mode =="esdfmRF_PLE_train_stream":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_v2(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        
        elif mode =="esdfmRF_ShareEmb_train_stream":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_v2(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        
        elif mode =="esdfmRF_train_stream_wo_delay_refund_backfill":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_wo_delay_refund_backfill(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        elif mode =="esdfmRF_train_stream_wi_refund_bias":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_wi_refund_bias(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        elif mode =="esdfmRF_train_stream_dp":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_dp(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days(args.test_split_days_start, args.test_split_days_end)
        elif mode =="esdfmRF_PLE_train_stream_and_test_inw_dp_samples":
            train_data = data_src.sub_days(0, args.train_split_days_end).add_esdfmRF_duplicate_samples_v2(pay_wait_window = pay_wait_window,refund_wait_window = refund_wait_window)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            train_data = train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            test_data = data_src.sub_days_inws_tn_pay(args.test_split_days_start, args.test_split_days_end,pay_wait_window)
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="oracle_pretrain", help='[defer_pretrain,defer_dp_pretrain,defer_train_stream,\
                        oracle_pretrain,oracle_train_stream, \
                        reoracle_pretrain,reoracle_train_stream,\
                        esdfm_pretrain,esdfm_tn_dp_pretrain,esdfm_train_stream,\
                        fnw_pretrain,fnw_train_stream,\
                        fnc_pretrain,fnc_train_stream,\
                        defuse_pretrain,defuse_tn_dp_pretrain,defuse_train_stream\
                        bidefuse_pretrain,bidefuse_train_stream,\
                        redefer_v0_pretrain,redefer_v0_dp_dr_pretrain,redefer_v1_pretrain,redefer_v1_dp_dr_pretrain,redefer_v1_train_stream,redefer_v2_train_stream\
                        refnc_pretrain,refnc_train_stream,\
                        vanilla_pay_inw_pretrain,vanilla_pay_inw_train_stream,\
                        vanilla_netpay_inw_pretrain,vanilla_netpay_inw_train_stream,\
                        ddfm_pretrain,ddfm_tn_dp_pretrain,ddfm_train_stream,\
                        refnw_pretrain,refnw_train_stream,\
                        reesdfm_pretrain,reesdfm_train_stream,\
                        reddfm_pretrain,reddfm_train_stream,\]')
