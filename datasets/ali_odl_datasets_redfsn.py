...
class DataDF(object):
    ...
    def add_redfsn_main_duplicate_samples(self, pay_wait_window,refund_wait_window):
        """
    ...
    def add_redfsn_main_duplicate_samples_wo_refund_backfill(self, pay_wait_window,refund_wait_window):
        """
        在给定归因窗口和观测窗口内，生成带有延迟标签的样本，并返回DataDF对象。注意此时我们不考虑退款
        dfsn 主塔有等待窗口，且回补延迟正样本
    ...
    def add_redfsn_feature_duplicate_samples(self, pay_wait_window,refund_wait_window):
        """
        在给定归因窗口和观测窗口内，生成带有延迟标签的样本，并返回DataDF对象。注意此时我们不考虑退款
    ...
    def add_redfsn_unbias_duplicate_samples(self):
        """
        添加假负样本以处理延迟转化。
        ...
        stream_pay_labels = np.concatenate([np.zeros_like(stream_pay_labels), np.ones((np.sum(pos_mask),)),np.zeros((np.sum(refund_mask),))],axis=0)

        stream_net_pay_labels = copy.deepcopy(self.net_pay_labels)
        stream_net_pay_labels = np.concatenate([np.zeros_like(stream_net_pay_labels), np.ones((np.sum(pos_mask),)),np.zeros((np.sum(refund_mask),))],axis=0)

        stream_pay_mask = np.concatenate([
        np.ones_like(pay_labels),  
    ...
    def add_redfsn_unbias_duplicate_samples_worefund(self):
        """
        添加假负样本以处理延迟转化。
        这里dfsn的无偏塔参考fnw的实现就好
        Args:
        ...
        stream_net_pay_labels = copy.deepcopy(self.net_pay_labels)
        stream_net_pay_labels = np.concatenate([np.zeros_like(stream_net_pay_labels), np.ones((np.sum(pos_mask),))],axis=0)
...
def get_ali_dataset_stream(args):
    ...
    if os.path.isfile(cache_path):
        ...
        if mode =="redfsn_train_stream":
            main_tower_train_data= data_src.sub_days(0, args.train_split_days_end).add_redfsn_main_duplicate_samples(pay_wait_window=pay_wait_window,refund_wait_window=refund_wait_window)
            main_tower_train_data= main_tower_train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            feature_tower_train_data= data_src.sub_days(0, args.train_split_days_end).add_redfsn_feature_duplicate_samples(pay_wait_window=pay_wait_window/2,refund_wait_window=refund_wait_window)
            feature_tower_train_data= feature_tower_train_data.sub_days(args.train_split_days_start, args.train_split_days_end)
            unbias_tower_train_data= data_src.sub_days(0, args.train_split_days_end).add_redfsn_unbias_duplicate_samples()
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="redfsn_pretrain", help='[redfsn_pretrain, redfsn_train_stream]')
