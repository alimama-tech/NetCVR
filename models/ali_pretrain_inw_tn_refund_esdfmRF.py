...
class PretrainInwTnRefundEsdfmRF(nn.Module):
    ...
    def predict(self, x):
        with torch.no_grad():
            ...
            inw_logits = output[:,0].view(-1)
            tn_logits = output[:,1].view(-1)
            inw_probs = torch.sigmoid(inw_logits)
            tn_probs = torch.sigmoid(tn_logits)
            return inw_probs, tn_probs
