...
class PretrainReDefer(nn.Module):
    def __init__(self, args):
        ...
        self.main_hidden = nn.Sequential(
            nn.Linear(128, 128),
            nn.BatchNorm1d(128),
            nn.LeakyReLU()
        )

        self.aux_hidden = nn.Sequential(
            nn.Linear(128, 128),
            nn.BatchNorm1d(128),
            nn.LeakyReLU()
        )
        ...
        self.smooth_factor_layer = nn.Linear(1,1)
    ...
    def forward(self, x):
        ...
        main_feats = self.main_hidden(feats)
        aux_feats = self.aux_hidden(feats)
        cvr_logits = self.head_main(main_feats)
        net_cvr_logits = self.head_aux(aux_feats)
        uncertainty_factor= self.smooth_factor_layer(torch.abs(net_cvr_logits-cvr_logits))
        uncertainty_factor = torch.sigmoid(uncertainty_factor)
        return torch.sigmoid(cvr_logits).squeeze(1), torch.sigmoid(net_cvr_logits).squeeze(1), uncertainty_factor.squeeze(1),main_feats,aux_feats
