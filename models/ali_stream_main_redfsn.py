...
class StreamMainReDfsn(nn.Module):
    ...
    def cvr_forward(self, x, feature_x_embed,unbias_x_embed):
        x = x.long()
    ...
    def net_cvr_forward(self, x, feature_x_embed,unbias_x_embed):
        x = x.long()
    ...
    def predict(self, main_outputs,feature_outputs,unbias_outputs,fused_alpha=1):
        with torch.no_grad():
            fused_outputs = torch.sigmoid((main_outputs + unbias_outputs)*fused_alpha)
