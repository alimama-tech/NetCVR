...
from trainers.ali_trainer_remiss import AliReMissStreamTrainer
from dataloaders.ali_odl_dataloader_remiss import get_ali_stream_dataloader
from models.ali_pretrain_remiss import PretrainReMiss
from models.ali_stream_remiss import StreamReMiss
def parse_args():
    ...
    parser.add_argument('--pretrain_split_days_end', type=int, default=14, help='end day of train (days)')
    ...
    parser.add_argument('--mode', type=str, default="remiss_train_stream", help='[remiss_train_stream]')
    ...
    parser.add_argument('--pretrain_miss_model_pth', type=str, default='/mnt/nfs/luomingxuan/airbench/main_exp3/Ali_refnc_pretrain.pth', help='if need a pretrain model to support')
...
if __name__ == '__main__':
    ...
    pretrain_model = PretrainReMiss(args).to(device)
    pretrain_model.load_state_dict(torch.load(args.pretrain_miss_model_pth, map_location=device),strict=False)
    stream_model = StreamReMiss(args).to(device)
    stream_model.load_state_dict(torch.load(args.pretrain_miss_model_pth, map_location=device),strict=False)


    stream_trainer = AliReMissStreamTrainer(args, pretrain_model,stream_model,train_stream_dataloader,  test_stream_dataloader, miss_global_pos_weight)
