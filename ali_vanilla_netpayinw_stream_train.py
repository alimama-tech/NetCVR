...
from trainers.ali_trainer_vanilla_netpay_inw import AliVanillaNetPayInwStreamTrainer
...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, default="vanilla_netpay_inw_train_stream", help='[vanilla_netpay_inw_train_stream]')
    ...
    parser.add_argument('--pretrain_vanilla_model_pth', type=str, default='/mnt/nfs/luomingxuan/airbench/main_exp3/Ali_refnc_pretrain.pth', help='if need a pretrain model to support')
...
if __name__ == '__main__':
    ...
    pretrain_model = PretrainVanillaNetPayInw(args).to(device)
    pretrain_model.load_state_dict(torch.load(args.pretrain_vanilla_model_pth, map_location=device),strict=False)


    stream_trainer = AliVanillaNetPayInwStreamTrainer(args, pretrain_model,train_loader,test_loader)
